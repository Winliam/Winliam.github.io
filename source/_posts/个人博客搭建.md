...
## 自定义域名
1. 阿里云域名注册购买
2. 买到后阿里云中继续设置域名解析，完成后等待10min等DNS生效(使得www.guohongming.cn和guohongming.cn都可以被访问到)
3. 在本地public文件夹添加CNAME文件(内容为guohongming.cn)后重新部署一次
4. 在云端repo的setting->pages页面将guohongming.cn添加到Custom domain并保存，不用管warning
...
## 生成站点地图以被搜索引擎检索到(暂未完成)
1. 安装npm sitemap包
```shell
npm install hexo-generator-sitemap --save
npm install hexo-generator-baidu-sitemap --save
```
2. 配置_config.yml并将URL换成自己的域名
```yml
sitemap: 
    path: sitemap.xml
baidusitemap:
...
## 更换自定义域名
1. 到阿里云/腾讯云买新域名
2. 到阿里云的域名解析服务中设置解析记录
3. 到github的pages页面中更改custom domain